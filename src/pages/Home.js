...
const Home = () => {
    let [data, setData] = useState(cardsObj.filter(card => card.carousel === undefined))
    let [display, setDisplay] = useState({ display: 'block' })
    let [result, setResult] = useState("Recommended")
    ...
    return (
        <div className="main-wraper">
            <Navigation color="#FFFFFF" color2="#5A698F" color3="#5A698F" color4="#5A698F" />
                ...
                <div className="container">
                    <div className="search">
                        <div className="searchInputs">
                            ...
                            <input placeholder="Search for movies or TV series..." onChange={filterData} type="text" />
                        </div>
                    </div>
                    <div style={display} className="carouselWraper">
                        <Responsive />
                    </div>
                    <div className='results'>
                        <h2>{result}</h2>
                        <div className="cards-container">
                            {data.map(item => <Card image={item.img} year={item.year} category={item.category} category2={item.category2} name={item.name} mark={item.marked} />)}
                        </div>
                    </div>
                </div>
            </div>
